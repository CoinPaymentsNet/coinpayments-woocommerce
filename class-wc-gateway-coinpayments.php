...
class WC_Coinpayments extends WC_Gateway_Coinpayments {
    ...
    // Register REST API endpoint for custom payment gateway
    add_action('rest_api_init', function() {
        register_rest_route('wc/v3', '/payments/coinpayments-gateway', array(
            'methods' => 'POST',
            'callback' => 'handle_coinpayments_gateway_payment',
        ));
    });

    function handle_coinpayments_gateway_payment(WP_REST_Request $request) {
...
add_action( 'woocommerce_blocks_loaded', 'coinpayments_gateway_block_support' );
function coinpayments_gateway_block_support() {
    ...
    require_once __DIR__ . '/includes/class-wc-coinpayments-gateway-blocks-support.php';
