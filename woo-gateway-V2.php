...
/*
 * Plugin Name: WooCommerce CoinPayments.net Gateway API Version 2.0
...
add_action( 'plugins_loaded', 'init_coinpaymentsV2_gateway', 0 );

/*
* Initiallize Gateway Instance
*/
function init_coinpaymentsV2_gateway() {


    add_filter( 'woocommerce_payment_gateways', 'wccoinpaymentsV2_add_gateway' );

    function wccoinpaymentsV2_add_gateway( $methods ) {
    	if (!in_array('WC_Gateway_CoinpaymentsV2', $methods)) {
				$methods[] = 'WC_Gateway_CoinpaymentsV2';
			}
			return $methods;
    }


    class WC_Gateway_CoinpaymentsV2 extends WC_Payment_Gateway {
	...
	global $woocommerce;
        ...
        $this->id           = 'coinpaymentsV2';
        //$this->icon         = apply_filters( 'woocommerce_coinpayments_icon', plugins_url().'/coinpayments-payment-gateway-for-woocommerceV2/assets/images/icons/coinpayments.png' );
$this->icon         = apply_filters( 'woocommerce_coinpayments_icon', plugins_url().'/icon.png' );
        $this->has_fields   = false;
        $this->method_title = __( 'CoinPayments Payment Gateway', 'woocommerce' );
	$this->method_description = __( 'Accept Multiple Crypto Currencies Via CoinPayments Gateway', 'woocommerce' );  
        $this->ipn_url   = add_query_arg( 'wc-api', 'WC_Gateway_CoinpaymentsV2', home_url( '/' ) );
		...
		// Actions
		add_action( 'woocommerce_receipt_coinpaymentsV2', array( $this, 'receipt_page' ) );
		add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );

		// Payment listener/API hook
		add_action( 'woocommerce_api_wc_gateway_coinpaymentsV2', array( $this, 'check_ipn_response' ) );

    }


    /**
     * DEPRECATED FUNCTION FOR REMOVAL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     */
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			...
			'title' => array(
							...
							'default' => __( 'CoinPayments.net Crypto Payment Gateway', 'woocommerce' ),
							'desc_tip'      => true,
						),
			'description' => array(
...
class WC_CoinpaymentsV2 extends WC_Gateway_CoinpaymentsV2 {
	public function __construct() {
		_deprecated_function( 'WC_CoinpaymentsV2', '1.4', 'WC_Gateway_CoinpaymentsV2' );
		parent::__construct();
	}
}
}
